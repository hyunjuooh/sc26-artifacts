...
int main(int argc, char** argv) {
    ...
    int* next_epoch_indices = (int *) malloc(total_global_samples * sizeof(int)); // for prefetch hint
    
    for (int i = 0; i < total_global_samples; i++) {
        global_indices[i] = i;
        next_epoch_indices[i] = i;
    }

    pdcid_t* reg_offset_array = NULL;
    pdcid_t* obj_id_array = (pdcid_t *) malloc((total_files / size) * sizeof(pdcid_t));
    ...
    if (rank == 0) {
        printf("--- Starting PDC Reader Benchmark ---\n");
    ...
    for (int epoch = 0; epoch < NUM_EPOCHS; epoch++) {
        ...
        shuffle_indices(next_epoch_indices, total_global_samples, BASE_SEED + epoch + 1);

        // if (rank == 0 || rank == 1) {
        //     printf("[EPOCH %d] [RANK %d] Shuffled Sequence (first 15): ", epoch + 1, rank);
            
        //     for (int k = 0; k < total_global_samples; k++) {
        //         printf("%d ", global_indices[k]);
        //     }
        //     printf("...\n"); 
        // }

        // if (rank == 0 || rank == 1) {
        //     printf("[EPOCH %d] [RANK %d] Next Epoch Shuffled Sequence (first 15): ", epoch + 1, rank); 
            
        //     for (int k = 0; k < total_global_samples; k++) {
        //         printf("%d ", next_epoch_indices[k]);
        //     }
        //     printf("...\n");
        // }
        ...
        int j = 0;
        for (int i = rank; i < total_global_samples; i += size) {
            int prefetch_idx = next_epoch_indices[i];
            char prefetch_rec_filename[512];
            sprintf(prefetch_rec_filename, "%s/dataset_%d", data_dir, prefetch_idx); 
            
            obj_id_array[j] = PDCobj_open(prefetch_rec_filename, pdc_id);

            // printf("%s %lld\n", prefetch_rec_filename, obj_id_array[j]);
            
            j++;
        }

        PDCregion_receive_prefetch_hint(obj_id_array, reg_offset_array, (total_files / size));
        PDCregion_prefetch_by_objid();

        for (int j = 0; j < (total_files / size); j++) {
            PDCobj_close(obj_id_array[j]);
        }
        ...
        if (rank == 0) {
            double exchange_time = data_exchange_end - data_exchange_start;
            printf("Epoch %d Dat Exchange | Time: %.4f sec \n", epoch + 1, exchange_time);
        }
    }
